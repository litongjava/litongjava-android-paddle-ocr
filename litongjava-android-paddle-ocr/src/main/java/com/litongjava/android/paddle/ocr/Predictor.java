package com.litongjava.android.paddle.ocr;
...
public class Predictor {
  ...
  protected boolean loadModel(Context appCtx, String modelPath, int cpuThreadNum, String cpuPowerMode) {
    ...
    if (!modelPath.substring(0, 1).equals("/")) {
      ...
      PaddleOcrUtils.copyDirectoryFromAssets(appCtx, modelPath, realPath);
    }
    if (realPath.isEmpty()) {
      return false;
    }
  ...
  public boolean runModel() {
    ...
    Bitmap scaleImage = PaddleOcrUtils.resizeWithStep(inputImage, Long.valueOf(inputShape[2]).intValue(), 32);
